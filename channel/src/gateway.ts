...
import { handleTukenInbound } from "./inbound.js";
...
export async function startTukenGatewayAccount(
  channelId: string,
  channelLabel: string,
  ctx: ChannelGatewayContext<ResolvedTukenAccount>,
) {
  const account = ctx.account;
  if (!account.configured) {
    throw new Error(`tuken is not configured for account "${account.accountId}"`);
  }
  ...
  const sendHeartbeatTick = async () => {
    ...
    try {
      ...
      if (!ctx.abortSignal.aborted) {
        console.warn(
          `[tuken] heartbeat failed for account=${account.accountId}: ${stringifyUnknownError(error)}`,
        );
      }
    } finally {
            ...
            try {
              ...
              if (runId) {
                try {
                  ...
                  console.warn(
                    `[tuken] failed to push run.error runId=${runId}: ${stringifyUnknownError(pushError)}`,
                  );
                }
              }
            }
