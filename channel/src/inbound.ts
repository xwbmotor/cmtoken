...
import { getTukenRuntime } from "./runtime.js";
import { normalizeTukenTarget } from "./target.js";
import type { HubInboundPayload, ResolvedTukenAccount } from "./types.js";
...
export async function handleTukenInbound(params: {
  channelId: string;
  channelLabel: string;
  account: ResolvedTukenAccount;
  config: OpenClawConfig;
  token: string;
  payload: HubInboundPayload;
}) {
  const runtime = getTukenRuntime();
  ...
  const target = normalizeTukenTarget(conversationId);
  if (!target) {
    return;
  }
  ...
  const warnPushFailure = (scope: string, error: unknown, detail?: Record<string, unknown>) => {
    ...
    console.warn(
      `[tuken] hub push failed (${scope}) runId=${runId} count=${pushFailureCount} error=${stringifyUnknownError(error)}${detailText}`,
    );
  };
  ...
  await runtime.channel.session.recordInboundSession({
    ...
    onRecordError: (error) => {
      throw error instanceof Error
        ? error
        : new Error(`tuken session record failed: ${String(error)}`);
    },
  });
    ...
    await runtime.channel.reply.dispatchReplyWithBufferedBlockDispatcher({
      ...
      dispatcherOptions: {
        ...
        onError: (error) => {
          throw error instanceof Error
            ? error
            : new Error(`tuken dispatch failed: ${String(error)}`);
        },
      },
      replyOptions: {
        onModelSelected,
