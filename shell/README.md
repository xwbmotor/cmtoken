...
为了优化网络传输带宽并提升重复打包的效率，打包系统内置了以下两项增量与缓存机制：

* **⚡ 构建增量缓存 (Compiler Cache)**：
  打包脚本 `build.mjs` 会在本地建立 `.cache/` 缓存目录，自动存放已下载的各平台免安装版 Node.js、Git 以及 OpenClaw 源码大包。
  * **快速重包**：后续重新执行打包时，脚本会直接命中缓存，**在 5-10 秒内即可瞬间完成所有平台的二次打包**，无需重复下载数百兆的底座文件！
  * **强制刷新**：若需要强制拉取最新的底座和依赖进行完全干净的打包，只需追加 `--fresh` 或 `--force` 参数：
    ```bash
    node shell/packet/build.mjs --all --fresh
    ```

* **🔄 增量升级分发 (Incremental Update)**：
  除了生成数上百兆的跨平台完整部署包（含 Node 和 Git 等底座），打包系统还会**自动将纯粹的独立插件编译产物 (`cmtoken-v1.0.0-prod.tgz` 与 `tuken-v0.6.0.tgz`) 单独提取并同步归集到 `shell/releases/` 目录下**。
  * 对于**已经安装或预装了 OpenClaw 的目标客户**，管理员只需分发这两个几十 KB 大小的插件包以及轻量版安装脚本，运行部署即可在 1 秒内完成**全自动增量升级与连通性激活**，无需重复传输和解压庞大的运行底座！
