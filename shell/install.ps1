...
if ($globalOpenClawOk -and !$hasOfflinePackage) {
    $isPluginOnly = $true
    log-success "检测到系统已具备全局/工作区 OpenClaw 服务，且本地无离线部署大包。"
    log-info "🚀 将自动进入【极轻量级插件独立安装模式】！无需下载庞大的平台运行底座。"
}
...
try {
  ...
  if (r.code === 200 || r.status === 'success') {
    const data = r.data || {};
    console.log('SUCCESS|' + (data.device_token || '') + '|' + (data.pair_token || ''));
  } else {
...
// 2. 写入 auth-profiles.json (激活中移 OAuth refresh token)
